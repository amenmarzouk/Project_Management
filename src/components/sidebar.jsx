...
const proj=useContext(projcontext)
  ...
  useEffect(() => {
   ...
   proj.setAllProjects(JSON.parse(data))
  ...
  useEffect(() => {
    ...
    localStorage.setItem("projects",JSON.stringify(proj.AllProjects))
  console.log("run");
...
<div className=" flex  flex-col gap-5 items-center">
        ...
        <button className=" h-10 w-30 bg-red-500 rounded-md cursor-pointer" onClick={()=>  nav('/add')}>+ Add Project</button>
        {proj.AllProjects &&proj.AllProjects.map((project) => (
